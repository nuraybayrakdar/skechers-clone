...
const validateNewPassword = () => {
  localState.newPasswordError =
    newPasswordModel.value.length < 8 ||
    newPasswordModel.value === oldPasswordModel.value;
};

const validateForm = () => {
  validateOldPassword();
  validateNewPassword();
  localState.disableChange =
    localState.oldPasswordError || localState.newPasswordError;
};
...
<template>
  <form
    class="flex-col text-center bg-white md:w-[75%] rounded-xl shadow-lg mx-auto sm:w-full relative py-7 mb-8"
    ...
    <div class="md:w-[65%] mx-auto my-10 sm:w-full">
      <span class="p-float-label mb-2">
        <InputText
          ...
          class="w-full"
          required
          :class="{ 'p-invalid': localState.oldPasswordError }"
        />
        <label for="OldPasswordInputText">Old Password</label>
      </span>
      <p
        v-if="localState.oldPasswordError"
        class="text-red-500 text-xs italic text-left"
      >
        Old password at least 8 characters
      </p>
    </div>
    <div class="md:w-[65%] mx-auto my-10 sm:w-full">
      <span class="p-float-label mb-2">
        <InputText
          ...
          class="w-full"
          required
          :class="{ 'p-invalid': localState.newPasswordError }"
        />
        <label for="NewPasswordInputText">New Password</label>
      </span>
      <p
        v-if="localState.newPasswordError"
        class="text-red-500 text-xs italic text-left"
      >
        New password can't be equal to old password
      </p>
    </div>
    <div class="text-right mr-5">
      <button
        class="bg-[#2B2E4A] rounded-full drop-shadow-lg text-white text-md h-9 w-[85px] opacity-40"
        :class="{ ableChange: !localState.disableChange }"
        :disabled="localState.disableChange"
      >
        Change
      </button>
    </div>
